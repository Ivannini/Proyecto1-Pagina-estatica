...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const reviewForms = document.querySelectorAll('[id^="reviewForm"]');

  starRatings.forEach(starRating => {
    const stars = starRating.querySelectorAll('.fas, .far');
  ...
  reviewForms.forEach(form => {
    form.addEventListener('submit', function(event) {
      ...
      const productId = form.id.replace('reviewForm', '');
      ...
      reviewElement.innerHTML = `
        <p>${reviewText}</p>
        <p>Calificación: ${'★'.repeat(reviewRating)}${'☆'.repeat(5 - reviewRating)}</p>
      `;
